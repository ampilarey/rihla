...
| T4 | ~~**Translations are `*_dv` columns plus a `locale` column.** The two mechanisms contradict each other — is a row a language, or does a row contain languages?~~ **Resolved** for `trips`, `guide_steps`, `hero_banners`, `why_sections` and `why_features` — one row, JSON per language, English fallback ([ADR 0001](adr/0001-how-content-is-translated.md)). Only `media` is still English-only, and it moves with the media-library work. | Adding Arabic to a converted table is a key, not a column. Per-locale slugs and per-locale SEO remain a translation table's job, which is where `packages` and `articles` go. |
