...
**Companion document:** [`WEBSITE_UPGRADE_PLAN.md`](WEBSITE_UPGRADE_PLAN.md) (§28 maps this document against it)
...
3. **`Booking` is the aggregate root** for a party's purchase; capacity is owned by `Departure` and protected with **pessimistic row locking plus a database-level invariant**, because the "last seat sold twice" race is the one bug that costs real money and real trust (§7, §22).
...
| **Booking** | The purchase and its lifecycle | `Booking`, `BookingTraveller`, `BookingLine`, `SeatHold`, `BookingStatusTransition` |
...
*Removed after migration:* `is_admin` (superseded by roles — see §18 and §23.4)

**`roles` / `permissions` / pivots** — from `spatie/laravel-permission` (§18).
...
**`departures`**
`id`, `ulid`, `package_id`, `code` (`UMR-STD-2026-03`), `date_start`, `date_end`, `status` (§6.4), `capacity_total`, `capacity_held`, `capacity_confirmed`, `booking_opens_at`, `booking_closes_at`, `currency`, `airline`, `route` (JSON), `tour_leader_id`, `scholar_id`, `operational_notes`, `cancelled_at`, `cancellation_reason`
*Invariant:* `capacity_held + capacity_confirmed <= capacity_total` — enforced in the database, not only in PHP (§7.4).
...
`id`, `payment_id`, `provider`, `provider_reference`, `provider_event_id` *(unique — the idempotency key, §11.5)*, `type` (`authorize|capture|callback|refund|reconcile`), `amount`, `currency`, `status`, `raw_response` (JSON, **redacted**), `occurred_at`
...
**`documents`**
`id`, `ulid`, `documentable_type`, `documentable_id` (traveller, booking or customer), `type` (`passport|national_id|photo|vaccination|visa|permit|ticket|invoice|other`), `status` (§12.3), `current_version_id`, `expires_at`, `is_sensitive`, `notes`
...
**`visa_applications`**
`id`, `ulid`, `reference` (`RIH-V-2026-0233`), `traveller_id`, `booking_id`, `visa_type` (`umrah|tourist|transit|visit`), `application_number`, `status` (§13.3), `submitted_at`, `decision_at`, `issued_at`, `expires_at`, `rejection_reason`, `document_id`, `assigned_to`, `notes`

**`nusuk_permits`**
`id`, `ulid`, `reference`, `traveller_id`, `booking_id`, `permit_type` (`umrah|rawdah|other`), `nusuk_account_linked_at`, `requested_at`, `status` (§14.3), `permit_reference`, `slot_at` (for Rawdah), `issued_at`, `expires_at`, `rejection_reason`, `document_id`, `assigned_to`, `requirements_snapshot` (JSON — which rule set applied, §14.5), `notes`
...
| What does a customer see about their travellers? | Everything operationally necessary — documents, visa status, itinerary — because they are legally responsible for the booking. Medical notes are restricted (§18.3). |
...
## 6. Package vs Departure, and the Booking Aggregate
...
**Consequence:** adding a traveller to a booking is a booking operation *and* a departure capacity operation. The two are coordinated inside one database transaction that locks the departure row (§22.2) — but the capacity invariant belongs to `Departure` and is enforced there.

### 6.3 Departure status
...
### 6.4 What must be snapshotted into a booking
...
> Snapshot the **masked** passport number (`A12****89`) in `traveller_snapshot`, never the full number. The full number lives once, encrypted, on `travellers` (§18.4).
...
| `PAID` | Paid in full |
| `READY_FOR_TRAVEL` | Paid **and** every active traveller is travel-ready (§7.3) |
...
### 7.4 Payment status is derived, not stored twice
...
### 8.3 The race condition, concretely
...
### 8.4 Defence 1 — pessimistic row lock
...
**This requires InnoDB.** It is why the production MySQL/MariaDB engine matters, and why SQLite (the local default) cannot validate this behaviour — concurrency tests must run against MySQL in CI (§27.1).

### 8.5 Defence 2 — database invariant
...
### 10.1 Three layers, provider-independent
...
### 10.4 Idempotency and callback safety
...
### 11.1 Versioning is the whole point
...
- **Passport expiry:** Saudi entry generally requires ≥6 months' validity from arrival. The platform must flag a passport expiring within 6 months of `departure.date_start` as blocking, and the **6-month window must be configuration, not a constant** (§14.5, §28 D-6).
- **Checksum** every version (SHA-256) — detects corruption and silent replacement.
- **Storage:** private disk only. Never `public/`. Access is exclusively through short-lived signed URLs issued after a policy check (§18.2).
...
### 12.1 Per traveller, never per booking
...
## 13. Nusuk / Permit Model

### 13.1 Why it is separate from Visa
...
### 13.3 Requirements must be configuration
...
### 17.3 Locale must move into the URL
...
| **Release expired holds** | Per hold: lock departure → release → expire booking → audit | Departure, one hold at a time |
...
## 22. Laravel Module Boundaries

### 22.1 Adapt, don't impose
...
### 22.3 Why not a package/module framework
...
- **Concurrency: 20 parallel requests for the last 5 seats sell exactly 5** — the defining test, and it must run against **MySQL**, not SQLite, since the lock semantics differ (§8.4)
...
| D-5 | Production MySQL/MariaDB version — does it support CHECK constraints? | Assumed MySQL 8.0.16+ / MariaDB 10.2+ | **High** — §8.5 defence 2 depends on it. **Verify before M6.** |
...
## 28. Alignment With `WEBSITE_UPGRADE_PLAN.md`

No change has been made to the master plan. Recommendations are listed for approval.

### 28.1 Compatible — no change needed

| Master plan | This document |
|---|---|
| §5.1 package/departure split | Fully specified here (§6, §4) |
| §5.2 booking flow, seat holds, overbooking prevention | §7, §8 |
| §5.3 BML Connect, instalments, offline methods | §10 — provider abstraction added |
...
| §9.3 `spatie/laravel-permission` | §18 — nine concrete roles and verb permissions |
| §10.4 security requirements | §18 |
| Appendix C data model sketch | Superseded by §4 in detail; no contradictions |

### 28.2 Conflicts found

| # | Conflict | Resolution |
|---|---|---|
| C-1 | **P0.5 requires `hreflang` and a two-locale sitemap, but locale lives in the session with no per-locale URLs.** The SEO work cannot function as specified. | Add locale-prefixed routing to P0 (or explicitly move P0.5 behind it). §17.3, D-7 |
| C-2 | Master plan §9.4 leaves the i18n approach open (translation table *or* JSON) | Firmed: hybrid, translation tables for the four content entities (§17.1) |
| C-3 | Appendix C shows `visa_status` inside a `permits` table and one row per traveller | Split into `visa_applications` and `nusuk_permits` — different systems, different lifecycles (§13.1) |
| C-4 | Appendix C's `bookings` has no snapshot fields | `package_snapshot`, `terms_version`, `traveller_snapshot` added (§6.4) |
| C-5 | Master plan implies booking status covers documents and visa | Booking status is commercial only; readiness is a computed projection (§7.1) |
| C-6 | Concurrency tests cannot run on the SQLite test setup in `phpunit.xml` | CI must add a MySQL job (§25.6) |

### 28.3 Missing dependencies the master plan does not name

1. **Locale-in-URL** before P0.5 SEO work (C-1).
2. **MySQL in CI** before the booking engine can be verified (§25.6).
...
5. **Money stored as integer minor units** — not stated anywhere in the master plan (§9.4 here).

### 28.4 Recommended changes to the master plan — for approval, not applied
...
| R-5 | Add "verify production MySQL/MariaDB version" to §13 decisions | Gates §8.5 |
...
**Remove/defer:** as §26, unchanged from master plan §9.5.

---

## 29. Final Architecture Review
...
Everything in §26 — event bus, event sourcing, lakehouse, multi-tenancy, marketplace, autonomous agents, generic rules/pricing engines, and the A12–A33 enterprise standards series.
...
| **S9** | Notification domain; wire the events from §19.2 | S5 |
...
**Changed:** none. No migration written, no model created, no existing behaviour touched. `WEBSITE_UPGRADE_PLAN.md` deliberately **not** modified — recommendations are in §28.4 awaiting approval.
...
Six, listed in §28.2. The material one is **C-1**: P0.5's `hreflang`/sitemap work cannot function while locale is session-based with no per-locale URLs.

### E. Recommended changes to the master plan

Eight, listed in §28.4 (R-1 … R-8), none applied.

### F. Open decisions requiring human approval

Twelve, listed in §27. The three that block estimation: **D-5** (production MySQL version → capacity constraint), **D-7** (locale in URL → SEO phase), **D-9** (Nusuk operating model → Phase 3 scope).
