...
| **5 — Knowledge & learning** | The differentiator ships | ~~Knowledge Centre (§7.1)~~ **— the editorial standard built as the state machine; no article ships, because the named reviewer does not exist yet**, ~~Ziyarah Guide with offline (§7.2)~~ **— the same gate, misconceptions as a first-class table, and a save-the-whole-guide control proved against a stopped server; no location ships, for the same reason**, ~~Learning Academy (§7.3)~~ **— the plan is arithmetic on the departure date, the quiz teaches rather than marks, and no module ships for the same reason as the other two**, Scholar Portal (§6.4), readiness score **— deliberately not a single score; named concerns instead, see §8.2**, full CRM + finance (§8.1, §8.4) | **10–12 weeks** |
