...
| D16 | ~~**Low**~~ **fixed** *(found during P0.5)* | Registration is open to anyone and creates a non-admin account. Until `3a72d0a` every such signup 500'd, which hid it. **Closed:** `/register` answers 404 unless `REGISTRATION_OPEN=true` (`config/security.php`). Nothing linked to it, staff are created with `admin:create`, and a pilgrim reaches their booking through a signed link — so the account it made held no role and reached nothing, while mailing whatever address was typed. | `routes/auth.php`, `RegisteredUserController` | Breeze default, never closed |
...
| D93 | ~~**High**~~ **fixed** | **A whole page still in the retired palette, and precached.** `public/offline.html` is a standalone document with its own inline stylesheet — no Tailwind, no `Brand::` — so the palette swap did not reach it: `body` stayed at `linear-gradient(135deg, #8E2653, #2E2621)` with `#D2A03C` ticks. The service worker precaches it, so it is genuinely served. `PwaTest` had a branding assertion, but it named one pre-wine blue, which kept passing through a change that retired a different colour entirely. Naming one dead colour only ever catches that colour. | `public/offline.html`, `tests/Feature/PwaTest.php` | A page that paints itself, guarded by an assertion about the wrong hex |
| D94 | ~~**High**~~ **fixed** | **The guard against a half-finished palette change was not told about the palette change.** `BrandColourTest::RETIRED` lists hexes that must not appear in a view. It was never given the wine and gold values, so `#DBD3CE` survived in `errors/layout.blade.php` — one literal missed inside a `:root` block whose other five were migrated — and `#5B524D` in the guide PDF. The list is the mechanism; not updating it is the same as not having it. It now carries every retired value and scans `public/` as well as the Blade directory. | `tests/Feature/BrandColourTest.php`, `resources/views/errors/layout.blade.php`, `resources/views/pdf/guide.blade.php` | A ratchet nobody tightened |
| D95 | ~~**High**~~ **fixed** | **A migration whose meaning moved after it ran.** `2026_09_18_170000_rebrand_stored_banner_colours` rewrote stored banner colours to `Brand::WINE` and `Brand::CREAM` — constants, not literals. It ran on test and production while those held `#8E2653` and `#FBF6EC`, stamping the rows *and the live column default*. Changing the constants moved the stylesheets and reached neither, so the homepage call-to-action would have deployed maroon on a violet site and every new banner would have been born maroon. The fresh-database test kept passing because `migrate:fresh` re-runs that migration with the current constants and never sees the state a deployed database is actually in. This is D11/D34 for the third time: colour held in the database, out of reach of a class sweep. | `database/migrations/2026_09_21_090000_recolour_stored_brand_values.php`, the 2026_09_18 migration frozen to literals | A migration pointed at a constant |
| D96 | ~~**High**~~ **fixed** | **The one icon nobody was reading, guarded by a check that reads nothing.** `public/favicon.ico` still carried the retired maroon brand — a `#8E2653` sail, a `#D2A03C` sail, an `#2E2621` hull — through the whole violet change, and went on being the icon every browser without SVG support shows, which is the icon in most bookmark bars. Every other icon was recut, because the tests on those read pixels; the only assertion this file ever had was `filesize() > 0`, which passes for any bytes at all. An `.ico` is a directory of images rather than one image, so GD cannot open the container and nobody had written the eight lines that walk it. Proved by restoring the old file and watching the new test name all four retired colours. | `public/favicon.ico` and the whole icon set, `tests/Feature/BrandMarkTest.php` | A size check standing in for a content check |
| D97 | ~~**High**~~ **fixed** | **The service worker pinned every image at whatever a phone saw first.** `sw.js` served `/images/`, `/fonts/` and `/js/` cache-first with no revalidation, and those paths keep their names across deploys — so `rihla-mark-inverse.svg` stayed at the pre-rebrand artwork on every returning device while the hashed stylesheet beside it updated normally. The result was the new palette wrapped around the old logo, visible only to people who had been to the site before: a fresh browser, CI and every screenshot looked right. `VERSION` was the only lever and it had not moved through the whole palette change. Mutable assets now use stale-while-revalidate with `event.waitUntil`; cache-first is kept for `/build/assets/`, the one thing Vite renames when it changes. Proved in a browser — install the worker, change a file on disk, reload: the old bytes came back every time before, the new ones after — and offline re-checked with the server genuinely stopped. | `public/sw.js`, `tests/Feature/PwaTest.php` | Cache-first on a URL that outlives its contents |
...
| Role | Token | Hex |
|---|---|---|
| Primary — CTAs, active nav, links | `wine-500` | `#5F498A` |
| Highlight — **dark grounds only** | `gold-500` | `#EFD34D` |
| The same gold where the ground is light | `gold-600` | `#A88C1F` |
| The booking action — **light grounds only** | `teal-600` | `#0A5754` |
| Body text and dark UI | `ink` | `#2E2245` |
| Page background | `cream` | `#FFFDF0` |
| Semantic | `success` / `warning` / `error` | `#0F7A54` / `#9E6A0D` / `#D92D20` |

White on the primary measures **7.48:1**. The token names are historical — `wine` holds the
violet and `gold` the chiffon — because renaming them would have touched every view to change
nothing a visitor can see.

Gold needs two values and the booking button needs a third colour for one reason, recorded in
[ADR 0007](adr/0007-two-colours-that-cannot-share-a-background.md): the two brand colours are
**7.00:1** apart, and two shapes each clearing 3:1 off one background must be 9:1 apart, so **no
background shows both**. Chiffon on cream is 1.46:1 — a button with no findable edge.
...
**Responsive image delivery — done, and the variants were already there.** Hero uploads have been writing `_768w`, `_1280w` and `_1920w` WebP files since Phase 2, and `HeroBanner::getResponsiveImageUrlsAttribute()` has been returning their URLs the whole time. **No view ever called it.** Every visitor on every device was served the full-size file — and on the homepage that file is the Largest Contentful Paint element, so a telephone on a Maldivian mobile connection downloaded a 1920-pixel photograph to show it 390 pixels wide. The variants existed; the `srcset` did not.
  ...
  `x-stored-image` now emits one, and it is the component the gallery and trip cards already used, so this arrived everywhere at once rather than as a second component beside the first. `sizes` is passed at every call site: without it a browser assumes the image is the full viewport width and fetches the largest file, which is the whole problem `srcset` exists to solve.

  **`App\Support\ResponsiveImage` asks the disk rather than assuming**, which is the difference between this and the accessor that was already here. That one builds its URLs by string manipulation; any image uploaded before variant generation existed yields three URLs that 404, and **a `srcset` of 404s is worse than none** — the browser picks the one it wants and gets nothing, where without it the original would at least have loaded. A path with no variants renders as a plain `<img>`.
