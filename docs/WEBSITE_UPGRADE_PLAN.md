...
| **4 — Operations & portals** — *in progress* | The journey runs on the platform | Journey planning & capacity (§8.2) — ~~rooming with conflict detection~~ and ~~the departure board~~ **both done; the allocator and the single readiness score deliberately not built, see §8.2**, operations (§8.3), Tour Leader Portal (§6.3), Family Portal (§6.2), safety & emergency (§6.5), notifications | **8–10 weeks** |
