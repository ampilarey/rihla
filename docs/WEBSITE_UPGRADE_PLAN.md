...
| **6 — Intelligence** | Decisions from data | ~~BI dashboard (§8.5)~~ **— done: nine of §10.5's thirteen KPIs computed, and the other four named as absences rather than proxied; "nothing to measure" is a state of its own, distinct from zero**, ~~forecasting~~ **— done: seat projection from booking pace, as a range and never a point, that declines below three comparable journeys rather than extrapolate**, ~~smart alerts~~ **— done: only the conditions no other screen watches, each naming who acts and where, and each shown only to somebody who can act on it**, pilgrim AI assistant (§9.6), staff drafting assistant, personalisation | **6–8 weeks** |
