...
**Content-Security-Policy is the next piece of work, and is now unblocked.** The 36 inline `onclick`/`onsubmit` attributes that made an enforcing policy impossible are gone (D76) — a nonce can vouch for a `<script>` block this application wrote, never for code living in an attribute. What remains: nonce the inline `<script>` blocks, decide `style-src` (admin-chosen colours are written into inline `style` attributes, and `style-src-attr` is not reliable enough across browsers to nonce them), and keep `'unsafe-eval'` until Alpine moves from its standard build to `@alpinejs/csp`, which compiles expressions without `new Function()`. A policy loose enough to allow `'unsafe-inline'` would look like protection while providing almost none, which is worse than having none — it is the header a checklist marks green. Doing it properly is its own piece of work: nonce the inline blocks, move the database colours onto CSS custom properties, then enforce. HSTS is sent without `includeSubDomains` or `preload`, because both extend the promise to hosts this application does not control and preload is effectively permanent; audit log for every admin action on bookings, payments and documents; dependency scanning in CI; secrets only in `.env` (the deploy webhook secret pattern already in place is the right model); documented backup **and tested restore**.
