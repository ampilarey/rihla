...
Still to come in this phase: the minimal CRM.
...
| **3 — Booking & payments** ⬅ **in progress** | Money online, spreadsheets retired | ~~booking domain + capacity invariant (§5.1, §5.2)~~ **— done**, ~~public booking flow (§5.2)~~ **— done, stopping at the seat hold: payment needs BML credentials**, ~~staff booking admin~~ **— done**, ~~waiting list with auto-promotion~~ **— done**, ~~document wallet with versioning (§5.5) [R-8]~~ **— done**, ~~visa applications (§5.4a)~~ **— done**, ~~Nusuk permits (§5.4b) + computed travel readiness~~ **— done**, ~~payments abstraction + bank transfer and cash (§5.3)~~ **— done, stopping at BML: merchant onboarding has not happened**, ~~Pilgrim Portal v1 (§6.1)~~ **— done**, ~~invoices and receipts (§5.3)~~ **— done**, ~~import of historical customers with duplicate detection~~ **— done**, Booking flow (§5.2), BML Connect (§5.3), instalments, invoices, document wallet **with versioning** (§5.5) **[R-8]**, **visa applications (§5.4a)** and **Nusuk permits (§5.4b)** as separate deliverables **[R-4]**, minimal CRM (§8.1), **import of historical customers/pilgrims from spreadsheets with duplicate detection** (companion §5.3), Pilgrim Portal v1 (§6.1) | **8–10 weeks** |
