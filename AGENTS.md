...
- **A list of exact strings catches only what it was told exactly — so assert the property, not the blacklist.** `BrandColourTest::RETIRED` scanned the two document PDFs through an entire palette change and reported green on `#6B6259`, because the list held `#6B6159` — **one digit different**. It was blind to `#666`, `#ddd` and `#999` in principle, because it matches six-digit strings and CSS also takes three. The fix is not a longer list: `test_a_pdf_template_names_no_colour_of_its_own` asserts that `resources/views/pdf/*.blade.php` contain **no hex literal of any length**, which is the property actually wanted (they read `App\Support\Brand`) and cannot go stale, because it names no colours. A value a template genuinely needs goes into `Brand` — that is why `Brand::RULE` and `Brand::GOLD_ON_LIGHT` exist. **Checking the render, not the source, is what found it:** decode the PDF's content stream and read the `rg`/`RG` operators back as hex (`zlib.decompress` each stream, then `([\d.]+) ([\d.]+) ([\d.]+) (rg|RG)`), because that is the only thing that shows what a reader is actually handed. It is how the guide PDF's step number turned out to be **white on the gold at 1.49:1** — the brand rule says gold takes ink and never white, and nothing in the source read as wrong.
...
- **An `encrypted` cast and `data:anonymise` are incompatible, and the break is delayed.** The scrub commands write their stand-ins through the **query builder** — `Anonymise::standIn()` goes straight into `DB::table(...)->update()` — so Laravel's `encrypted` cast decrypts perfectly right up until somebody scrubs, and every read afterwards throws `DecryptException`. On the one command whose job is keeping real passport numbers off a public host. The answer is the pattern `App\Support\EncryptedFile` already uses for files: **every payload says which it is**. `App\Casts\EncryptedIdentifier` prefixes ciphertext and hands back anything unprefixed untouched, so three cases read correctly — ciphertext it wrote, a plaintext stand-in a command wrote, and a row predating the cast. It also returns null rather than throwing for ciphertext this `APP_KEY` cannot open (a restored database, a rotated key): a register missing one number is visible and fixable, where a page that 500s on load says nothing.

- **A "will be handled when Phase N lands" comment is a defect with a delivery date, and nothing collects it.** `ForgetCustomer` matched payments on `payable_type = Booking` with a note saying a stay's money would be reached *"when Phase 9 gives it a customer of its own"*. Phase 9 did exactly that and nothing came back to the line, so a guesthouse deposit would have survived a right-to-erasure request **that reported itself honoured** — the worst shape of privacy bug, because the report says it worked. Found by reading the comment, not by any test failing. When deferring like this, leave a failing-by-construction guard rather than prose: a test asserting the *current* narrow behaviour with a name that states the debt, so the phase that lands has to delete it deliberately.
...
- **Making a column polymorphic silently drops its `ON DELETE` behaviour, and a dotted Filament column name on a `MorphTo` throws.** `notices.booking_id` was `constrained()->cascadeOnDelete()`, so the database removed a booking's notices with the booking. `noticeable_type`/`noticeable_id` carries no constraint, so nothing does that any more — **nothing errors; the rows simply start surviving their owner**, in a table a deletion request has already reported clean. Rebuild the cascade in PHP (`App\Models\Concerns\HasNotices` does it in a `deleting` hook, in one place, so a third owner cannot be added without it) and assert it **for every owner** — getting one right and leaving the other is how this ships. Two more things go with it, both already recorded here in their other forms: the erasure route (`Forgetting::REACHED` said `booking`, which after the conversion is a query against a column that no longer exists — and the quiet half is the *other* owner's rows never being reached at all), and Filament, where `TextColumn::make('noticeable.customer.name')` makes the table walk the dotted path as a relationship to work out sorting and searching, calls `getRelated()` on a `MorphTo` that has no single related model, and throws inside the table view — **supplying `->state()` does not prevent it**, because the walk happens on the name. A column name with no dot in it is never walked.
...
- **A column added to a table an admin screen already lists shows as the screen's "nothing" placeholder, not as missing.** `enquiries` grew `stay_id`/`property_id` so a lost guesthouse ask becomes a follow-up. The CRM board's About column reads `package.title`, and a follow-up has no package — so every one of them rendered the column's placeholder, *"Not sure yet"*, about an enquiry where the person had named the property, the dates and the party size. Nothing failed: the page answered 200, the row was there, and the wrong words were the ones the screen was designed to print. **A new way of creating a row means re-reading every column of every screen that lists that table**, because a column written for one origin has no way to say it does not apply. Test it through `Livewire::test(ListX::class)` — the column is a closure, and per the Pulse note above a Filament page answers 200 while a closure throws in its own request.

- **A generator with no counterpart leaks, and nothing reports it.** `CoverImageObserver` wrote `_768w/_1280w/_1920w` WebP variants on every save of four models from Phase 2 onward, and the only deletion anywhere in the codebase was `HeroBannerController::deleteImage()`. So a **trip, package or article** cover that was replaced or deleted left three files on disk every time, for as long as the feature had existed. **Nothing could have caught it from the outside**: the site renders correctly with orphans present, every test passes, and the sole symptom is a disk slowly filling — on cPanel a fixed per-account allowance, where running out fails *writes across the whole application*, not just uploads. The lesson generalises past images: when you add something that creates files, rows or external records on one event, write the counterpart on the opposite event in the same commit, because the asymmetry is invisible until the resource runs out. Note also which test **must not** fail when you do: `an unrelated edit leaves the variants standing` — a cleanup keyed on the wrong condition deletes the variants of a cover nothing replaced, which looks identical to never having generated them.
...
- **Lighthouse gates CI now** (`lighthouse-budget.json`, `scripts/lighthouse-check.mjs`). Accessibility and SEO must be 100 on the homepage, packages list and guide; page weight is capped at 900 KB. Performance is reported and does not fail, because it is timing on a shared runner. Run it locally with `CHROME_PATH=/opt/pw-browsers/chromium-1194/chrome-linux/chrome node scripts/lighthouse-check.mjs http://127.0.0.1:8000`, after `php artisan db:seed --class=StaysShowcaseSeeder` — two of the five budgeted URLs are stays pages and they 404 until that seeder switches the strand on. **The audit retries once when Chrome will not start, and that cannot hide a regression:** a page scoring below its threshold exits *zero* and writes a report, so a budget failure never reaches the retry — the only thing that throws is the measurement failing to happen at all ("Unable to connect to Chrome" on a shared runner, with the site up and serving). Re-running a measurement that was never taken is not the same as re-rolling one that came back bad, and that distinction is what makes the retry safe to have.
