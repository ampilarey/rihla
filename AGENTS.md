...
- **Larastan needs the generic on a relation, and PHPStan only runs in CI.** A `HasMany` with no `@return HasMany<Child, $this>` types its collection as `Model`, so every closure taking the real class is an `argument.type` error and any method returning it is a `return.type` error — none of which `php artisan test` can see. The same applies to a model scope called on a bare `Illuminate\Database\Eloquent\Builder`: an arrow function cannot carry a docblock, so the scope has to move into a named method with `@param Builder<Model>`. `StaticAnalysisHabitsTest` guards the scope case locally; the relation case is caught only by CI, so declare the generic when you write the relation.
