...
- **Dropping a column on SQLite needs the indexes and the foreign key gone first, in separate statements.** `dropConstrainedForeignId()` bundles the foreign key and the column into one blueprint and leaves indexes alone; SQLite rebuilds the whole table for any of these, and the rebuild fails on whatever still names the departing column — *"error in index … after drop column"* or *"unknown column … in foreign key definition"*. The order that works on both engines: drop each index naming the column, then `dropForeign` (**on SQLite too** — it keeps the constraint in the table definition), then `dropColumn`, each in its own `Schema::table()` call. Getting this wrong half-applies the migration and leaves it unrecorded, so the next run fails on a duplicate column instead of the real cause — always diagnose with `migrate:fresh`, never against a database a failed attempt has already touched.
