...
Rihla Travels is a Laravel 12 (PHP 8.2+) travel website using TailwindCSS + Vite + Alpine.js on the frontend. It defaults to a SQLite database (`database/database.sqlite`). There is a single deployable web app (public site + `/admin` panel), so no multi-service orchestration is needed.
