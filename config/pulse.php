...
/*
 * Pulse, on cPanel shared hosting.
 *
 * Three of Pulse's defaults assume a host this application does not have
 * (ADR 0002), and are changed below rather than left to fail quietly:
 *
 *  - The ingest driver stays `storage`, which writes during the request's
 *    terminating phase. The Redis ingest is the documented way to keep that
 *    off the request path, and there is no Redis here.
 *  - `pulse:check` is a daemon, and nothing long-running can be kept alive on
 *    this host. The Servers recorder is therefore commented out, and the
 *    Servers card is not on the dashboard, rather than shipping a panel that
 *    is empty forever.
 *  - Trimming rides the ingest lottery (1 in 1,000 requests), not the
 *    scheduler, so the `pulse_*` tables stay bounded without a cron entry.
 *
 * Everything Pulse does is wrapped in its own rescue(), so a failure here
 * cannot take a page down with it.
 */
...
return [
    ...
    */
    ...
    'recorders' => [
        // Off by default here, which is the one recorder default this file
        // reverses. The cache driver on this host *is* the database
        // (ADR 0002), and the homepage caches its "Why Choose Rihla" block —
        // so leaving this on writes a Pulse row to MySQL for every cache read
        // from MySQL, on every page view, to report on how the cache is
        // doing. That is a loop, and it is the only recorder here that fires
        // on an ordinary anonymous page view. Set PULSE_CACHE_INTERACTIONS_ENABLED=true
        // for a spell of diagnosis, then turn it back off.
        ...
        // Fed only by `pulse:check`, which is a daemon. cPanel has no way to
        // keep one alive — that is the first row of ADR 0002's table — so the
        // recorder is registered nowhere and the Servers card is off the
        // dashboard. Restore both together if this ever moves to a VPS:
        //
