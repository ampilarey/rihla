...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Guide Step</h1>
                <p class="mt-2 text-gray-600">Update step information and content</p>
            </div>
        ...
        <form action="{{ route('admin.guide-steps.update', $guideStep) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="locale" id="locale" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent">
                            <option value="">Select Language</option>
                            <option value="en" {{ old('locale', $guideStep->locale) === 'en' ? 'selected' : '' }}>English</option>
                            <option value="dv" {{ old('locale', $guideStep->locale) === 'dv' ? 'selected' : '' }}>Dhivehi</option>
                        </select>
                        @error('locale')
                    ...
                    <div>
                        ...
                        <input type="number" name="step_number" id="step_number" min="1" value="{{ old('step_number', $guideStep->step_number) }}" required
                ...
                <div class="mt-6">
                    ...
                    <input type="text" name="title" id="title" maxlength="120" value="{{ old('title', $guideStep->title) }}" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent"
                ...
                <div class="mt-6">
                    ...
                    <textarea name="summary" id="summary" rows="3" required
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent"
                              placeholder="Short summary of this step (1-2 paragraphs)...">{{ old('summary', $guideStep->summary) }}</textarea>
                    @error('summary')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="details" id="details" rows="4" 
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent"
                              placeholder="Expanded content and additional details...">{{ old('details', $guideStep->details) }}</textarea>
                    @error('details')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6">
                    ...
                    <input type="file" name="image" id="image" accept="image/*" 
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent">
                    <p class="mt-1 text-sm text-gray-500">Recommended: 1200x900px, max 6MB, supports JPEG, PNG, WebP. Leave empty to keep current image.</p>
                    @error('image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="dua_text" id="dua_text" rows="3" 
                                  class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent"
                                  placeholder="Supplication text for this step...">{{ old('dua_text', $guideStep->dua_text) }}</textarea>
                        @error('dua_text')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $guideStep->video_url) }}"
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent"
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div id="checklist-container" class="space-y-3">
                            @if($guideStep->hasChecklist())
                                @foreach($guideStep->checklist_items as $item)
                                    <div class="flex gap-3">
                                        <input type="text" name="checklist[]" value="{{ $item }}"
                                               class="flex-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent"
                    ...
                    <div>
                        ...
                        <div id="fiqh-notes-container" class="space-y-3">
                            @if($guideStep->hasFiqhNotes())
                                @foreach($guideStep->fiqh_notes as $note)
                                    <div class="flex gap-3">
                                        <input type="text" name="fiqh_notes[]" value="{{ $note }}"
                                               class="flex-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-sky-blue focus:border-transparent"
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $guideStep->is_published) ? 'checked' : '' }}
            ...
            <div class="flex justify-end gap-4">
                ...
                <button type="submit" 
                        class="px-6 py-3 bg-brand-sky-blue hover:bg-blue-600 text-white font-medium rounded-lg transition-colors duration-200">
                    Update Guide Step
                </button>
            </div>
        </form>
