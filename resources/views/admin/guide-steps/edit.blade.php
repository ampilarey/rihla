    ...
    <div class="max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.guide-steps.update', $guideStep) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <x-admin.translatable-field name="title" label="Title" :value="old('title', $guideStep->getTranslations('title'))" required
                    help="Maximum 120 characters." />

                <x-admin.translatable-field name="summary" label="Summary" :value="old('summary', $guideStep->getTranslations('summary'))" required
                    type="textarea" :rows="3" help="Shown on the step card — one or two sentences." />

                <x-admin.translatable-field name="details" label="Details (optional)" :value="old('details', $guideStep->getTranslations('details'))"
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Recommended: 1200x900px, max 6MB, supports JPEG, PNG, WebP. Leave empty to keep the current image.</p>
                    @error('image')
                        <p class="mt-1 text-sm text-error">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <x-admin.translatable-field name="reference_text" label="Reference (Optional)"
                    :value="old('reference_text', $guideStep->getTranslations('reference_text'))" type="textarea" :rows="2"
                    help="Qur'an and hadith this step is based on." />

                <x-admin.translatable-field name="fiqh_notes" label="Fiqh Notes (Optional)"
                    :value="old('fiqh_notes', $guideStep->getTranslations('fiqh_notes'))" type="lines" :rows="4"
                ...
                <x-admin.translatable-field name="checklist" label="Checklist (Optional)"
                    :value="old('checklist', $guideStep->getTranslations('checklist'))" type="lines" :rows="4"
